...
- Aldea en mundo infinito por chunks: `VillageState` es `{seed, chunks}` (`state/village.ts`), no un grid fijo. Cada chunk es de 16x16 tiles y se genera proceduralmente la primera vez que el jugador se acerca, con un PRNG determinístico (mulberry32) sembrado por `worldSeed + coordenadas del chunk` — el mismo chunk siempre regenera el mismo contenido, así que no hace falta guardarlo tile por tile, alcanza con persistir qué chunks existen y su estado (nodos ya golpeados/agotados). `ensureChunksLoaded` corre cada frame y genera los chunks alrededor del jugador (radio 2 chunks) — es barato porque es un lookup en un `Record`, salvo la primera vez que se pisa un chunk nuevo. Los chunks nunca se descargan de memoria en la sesión actual (limitación conocida y aceptada: para sesiones de juego normales no pesa, pero si en algún momento se vuelve un problema real, lo próximo sería eviction + diff de chunks lejanos).
- Cámara centrada en el jugador: el canvas es un viewport fijo (24x20 tiles) que sigue al jugador, no el mapa entero. Tanto el render (`engine/renderer.ts`) como las consultas de gameplay (`getNodesInTileRange`/`getNodesNearTile` en `state/village.ts`) están acotadas a los tiles/chunks visibles o cercanos, así el costo por frame no crece aunque el jugador explore muchísimo mundo — es la solución al lag que tenía el approach anterior de grid fijo. Coordenadas del jugador dibujadas in-game como overlay en la esquina superior izquierda del canvas (`drawCoordsOverlay` en `engine/renderer.ts`), no en el HUD lateral — quedan siempre a la vista sin ocupar espacio en la sidebar.
...
- Autoguardado en localStorage cada ~3s (save key `wanderhold-save-v7`, bumpeada por el cambio de `VillageState` a mundo infinito por chunks — forma totalmente distinta a la anterior, no hay migración posible, saves de versiones previas se descartan solos).
